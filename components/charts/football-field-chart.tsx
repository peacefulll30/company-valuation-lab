 ...
 * horizontal bar per estimate. Comps-implied range and a current-price
 * marker are omitted here (both require market price, which is honestly
 * unavailable this phase) — shown "where sufficient data exists" per the
 * Phase 4 brief, not blended with a fabricated range.
 */
export function FootballFieldChart({ ranges }: { ranges: FootballFieldRange[] }) {
  const data = ranges.map((r) => ({ ...r, base_offset: r.low, base_width: r.high - r.low }));

  return (
    <ChartCard
      title="Valuation range"
      question="Where does the DCF range sit, high to low?"
      chart={
        <div className="h-40">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart
              ...
              desc="Horizontal bar chart showing the DCF-implied valuation range. Use the arrow keys to move between estimates; the tooltip shows the low, base, and high values."
            >
